	...
	*/
	...
	protected static void doLogin(Stage ts) {
    	...
    	User user = new User(username, password, theDatabase.getCurrentFirstName(), 
    			...
    			theDatabase.getCurrentStudentRole(), theDatabase.getCurrentReviewerRole());
		...
		if (numberOfRoles == 1) {
			...
			// Reviewer role
				...
				loginResult = theDatabase.loginReviewer(user);
				if(password.length() == 6) {
					guiUserUpdate.ViewUserUpdate.displayUserUpdate(ts, user, false);
				}
				else if (loginResult) {
					guiReviewer.ViewReviewerHome.displayReviewerHome(theStage, user);
				}
				// Other roles
			} else {
